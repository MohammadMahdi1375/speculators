...
class RemoteTarget:
    ...
    def __init__(self, draft, endpoint, model, expected_layers, timeout=180):
        ...
        # The supervised target is local. Avoid sending localhost traffic through
        # a proxy inherited from the shell (including ALL_PROXY/SOCKS settings).
        options = {}
        if urlsplit(endpoint).hostname in {"127.0.0.1", "localhost", "::1"}:
            options["http_client"] = DefaultHttpxClient(trust_env=False)
        self.client = OpenAI(base_url=endpoint, api_key="EMPTY", timeout=timeout, **options)
    ...
    def states(self, tokens):
        ...
        try:
            ...
            # Trainable draft tensors are FP32 optimizer masters; frozen verifier
            # weights remain BF16. Do not let the first trainable parameter choose
            # the dtype of target features or verifier scoring.
            hidden = hidden.to(device=self.draft.embed_tokens.weight.device,
                               dtype=self.draft.embed_tokens.weight.dtype)
