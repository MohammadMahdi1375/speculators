...
from .continuous_training import CohortRollouts, ContinuousRollouts
from .clean_training import backward_trajectory
from .execution import make_prompt_queue, static_prompt_groups, validate_prompt_coverage
from .performance import peak_memory_gib, synchronize
from .utilization_resume import check_checkpoint, training_signature, validate_resume_signature
...
def main():
    ...
    if args.vllm_endpoint:
        parser.error("This execution experiment uses a local target on each trainer NPU")
    if args.rollout_scheduler == "continuous" and args.performance_mode != "cached":
        parser.error("Continuous rollouts require --performance-mode cached")
    ...
    if args.resume:
        ...
        execution_changes = validate_resume_signature(
            resumed["signature"], signature, args.allow_performance_change
        )
    ...
    try:
        for batch_index, (epoch, batch) in enumerate(
            ...
            local = [0.0] * 11
            claim_size = 1 if args.rollout_scheduler == "continuous" else args.rollout_batch_size
            groups = (
                queue.groups(batch, step + 1, claim_size)
                if queue is not None
                else static_prompt_groups(batch, rank, world, claim_size)
            )
            collector = (
                ContinuousRollouts(target, draft, groups, prompts, args.response_length,
                                   eos, beta, max_active=args.rollout_batch_size,
                                   trace_storage=args.trace_storage)
                if args.rollout_scheduler == "continuous"
                else CohortRollouts(target, draft, groups, prompts, args, eos, beta)
            )
            ...
            while True:
                ...
                draft.eval()
                with amp():
                    completed = collector.next_ready()
                if completed is None:
                    break
                batch_positions, indices, collected = completed.positions, completed.indices, completed.batch
            ...
            if rank == 0:
                ...
                row = {
                    ...
                    "rollout_batch_size": args.rollout_batch_size,
                    "rollout_scheduler": args.rollout_scheduler,
                    "mean_active_rollouts_per_draft_forward": values[2] / max(1, values[10]),
                    "attention_backend": args.attention_backend,
                    "target_norm": args.target_norm,
                    "work_distribution": args.work_distribution,
                    "beta": beta,
