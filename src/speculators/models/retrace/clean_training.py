...
def backward_trajectory(
    ...
    *,
    ...
    for start in range(0, len(trace.positions), blocks_per_forward):
        ...
        if optimized:
            # Memory has a fixed FP16 storage format, including in BF16 runs.
            # Autocast must not reinterpret the storage concatenation (CPU
            # BF16 autocast rejects FP16 cat); only network math uses autocast.
            with torch.autocast(device_type=device.type, enabled=False):
                memory = ReTraceMemory(
                    torch.cat([x.draft for x in chunk]).to(
                        device=device, dtype=forward_dtype
                    ),
                    torch.cat([x.target for x in chunk]).to(
                        device=device, dtype=forward_dtype
                    ),
            ...
            memory = ReTraceMemory(
                torch.cat(
                    [x.draft.to(device=device, dtype=forward_dtype) for x in chunk]
                ),
                torch.cat(
                    [x.target.to(device=device, dtype=forward_dtype) for x in chunk]
                ),
                torch.cat([x.valid for x in chunk]).to(device),
            )
