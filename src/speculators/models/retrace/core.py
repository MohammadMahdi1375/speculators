...
@SpeculatorModel.register("retrace")
class ReTraceDraftModel(DFlashDraftModel):
    ...
    def forward(
        ...
        if context_hidden is None or anchor is None:
            raise ValueError(
                "ReTrace requires live proposal verification. Use python -m speculators.models.retrace.train; stock teacher-forced batches contain no rejected trajectory."
            )
