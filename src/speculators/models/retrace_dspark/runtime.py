...
def setup(args, device):
    ...
    if args.draft:
        saved = ReTraceDSparkSpeculatorConfig.from_pretrained(
            args.draft, local_files_only=True
        )
        if saved.speculators_model_type != "retrace_dspark":
            raise ValueError(
                "Use a retrace_dspark checkpoint. DFlash-based retrace and legacy combined checkpoints are incompatible."
            )
        saved.speculators_config.verifier.name_or_path = args.target
        draft = ReTraceDSparkDraftModel.from_pretrained(
        ...
        saved = ReTraceDSparkSpeculatorConfig(
