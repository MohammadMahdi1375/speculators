"""Strict, local-only import of the public Qwen3 DSpark drafter.
...
def source_config(source, target):
    ...
    if kind == "dspark":
        config = DSparkSpeculatorConfig.from_pretrained(source, local_files_only=True)
        ...
        if (
            not config.sample_from_anchor
            or config.draft_vocab_size != config.target_vocab_size
        ):
            raise ValueError(
                "Expected full-vocabulary DSpark with sample_from_anchor=True"
            )
        convention = "speculators HF hidden-state indices, copied unchanged"
    elif kind is None and raw.get("architectures") == ["Qwen3DSparkModel"]:
        if raw.get("model_type") != "qwen3":
            raise ValueError("Expected the official dense Qwen3 DSpark format")
        required = (
            "target_layer_ids",
            "mask_token_id",
            "block_size",
        ...
        if any(name not in raw for name in required):
            raise ValueError(
                "Official DSpark config is missing architecture/head fields"
            )
        drop = {"architectures", "auto_map", "num_target_layers", *required}
        tl = AutoConfig.for_model(
            "qwen3", **{k: v for k, v in raw.items() if k not in drop | {"model_type"}}
        )
        layer_ids = [int(i) + 1 for i in raw["target_layer_ids"]]
        block_size, mask_id = int(raw["block_size"]), int(raw["mask_token_id"])
        heads = {name: raw[name] for name in required[3:]}
        if (
            raw.get("num_target_layers", target_config.num_hidden_layers)
            != target_config.num_hidden_layers
        ):
            raise ValueError("DSpark was built for a different target depth")
        convention = "DeepSpec decoder-layer indices + 1 = HF hidden-state indices"
    else:
        raise ValueError(
            "Use the original DSpark checkpoint, not DFlash or an existing ReTrace checkpoint"
        )
    if (
        heads["markov_head_type"] != "vanilla"
        or int(heads["markov_rank"]) <= 0
        or heads["enable_confidence_head"] is not True
        or heads["confidence_head_with_markov"] is not True
    ):
        raise ValueError(
            "This recipe requires vanilla Markov and Markov-conditioned confidence heads"
        )
    for name in (
        "hidden_size",
        "vocab_size",
    ...
    config = ReTraceDSparkSpeculatorConfig(
        **common,
        training_objective="dspark_clean_block",
...
def inspect_source(source, target):
    ...
    return config, {
        ...
        "proposals_per_round": config.block_size,
        "draft_layers": config.transformer_layer_config.num_hidden_layers,
        "tensor_shapes": keys,
    }


def load_dspark(source, target, dtype=torch.bfloat16, load_shared=True):
    ...
    with torch.random.fork_rng(devices=[]):
        ...
        draft = ReTraceDSparkDraftModel(config).to(dtype=dtype)
    expected = {
        name
            ...
            with safe_open(path, framework="pt", device="cpu") as handle:
                for name in handle.keys():
                    if name in SHARED:
                        if not load_shared:
                            raise ValueError(
                                "Shared source tensors require target validation"
                            )
                        value = handle.get_tensor(name)
                        if value.shape != parameters[name].shape or not torch.equal(
                            parameters[name], value.to(dtype)
                        ):
                            raise ValueError(
                                f"Shared pretrained tensor differs from frozen target: {name}"
                            )
                        shared_checked.append(name)
                        continue
    ...
    report.update(
        ...
        shared_tensors_verified=shared_checked,
        heads_preserved=[
            n
            for n in sorted(loaded)
            if n.startswith(("markov_head.", "confidence_head."))
        ],
...
def prepare(source, target, output):
    ...
    draft, report = load_dspark(source, target)
    output.mkdir(parents=True)
    draft.save_pretrained(output / "retrace_dspark")
    ...
    for key in list(raw):
        if key.startswith("retrace_") or key in {
            ...
            raw.pop(key)
    raw["speculators_model_type"] = "dspark"
    raw["architectures"] = ["DSparkSpeculator"]
    raw["speculators_config"]["algorithm"] = "dspark"
    baseline = DSparkSpeculatorConfig(**raw)
    baseline_dir = output / "dspark"
    baseline_dir.mkdir()
