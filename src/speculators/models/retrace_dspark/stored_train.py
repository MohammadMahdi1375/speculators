"""Train stored-response ReTrace with the existing DSpark data loader/Trainer."""
...
from .stored_training import ReTraceDSparkDraftModel
...
def load_model(checkpoint):
    config = ReTraceDSparkSpeculatorConfig.from_pretrained(checkpoint)
    config.transformer_layer_config._attn_implementation = "sdpa"
    config.training_objective = "dspark_stored_pair"
    model = ReTraceDSparkDraftModel.from_pretrained(
...
def main():
    ...
    setup_root_logger()
    setup_metric_logger(
        [], run_name="retrace_dspark_stored", output_dir=str(output / "logs")
    )
