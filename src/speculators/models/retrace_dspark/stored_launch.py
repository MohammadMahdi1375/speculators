"""Supervise vLLM target NPUs and standard DSpark trainers for stored ReTrace."""
...
from .pretrained_launch import stop_group
...
def main():
    ...
    for name in ("root", "target", "base-dspark", "data", "output"):
    ...
    for path in (
        ...
        Path(args.base_dspark) / "config.json",
    ):
        if not path.is_file():
    ...
    run.update(
        ...
        root=str(root),
        endpoint=endpoint,
        served_model="retrace-dspark-stored-target",
        stored_data=str(output / "stored_data"),
        initial_checkpoint=str(output / "initial/retrace_dspark"),
        protocol="dspark_stored_pair_v1",
        ...
        objective="DSpark CE(0.1) + TV(0.9) + confidence BCE(1), exp(-proposal_index/4)",
        batch_unit="packed tokens per worker; not global 32 prompts",
        pretrained_backbone="DSpark-block7; Markov and confidence heads retained",
    ...
    if not args.resume:
        ...
        prepare_weights(args.base_dspark, args.target, output / "initial")
        config = ReTraceDSparkSpeculatorConfig.from_pretrained(
            output / "initial/retrace_dspark"
        )
        if config.block_size != 7:
            raise ValueError(
                "This recipe requires the original DSpark-block7 checkpoint"
            )
        ...
        print(
            ...
            f"Retention {'enabled' if args.cache_clean else 'disabled (DSpark delete mode)'}. ",
            flush=True,
        )
        if args.cache_clean:
    ...
    trainer_command = [
        ...
        "-m",
        "speculators.models.retrace_dspark.stored_train",
        "--run-config",
    ...
    wrapper = root / "speculators/examples/train/train_retrace_dspark_stored_vllm.sh"
