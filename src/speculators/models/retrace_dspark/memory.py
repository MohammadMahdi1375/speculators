...
class ReTraceRequestCache:
    ...
    def __init__(self, proposals: int, hidden_size: int):
        self.k = proposals
    ...
    def inputs(
        ...
        m = torch.zeros(n, k, self.hidden_size, device=device, dtype=dtype)
    ...
    def capture(self, hidden: torch.Tensor, positions: torch.Tensor) -> None:
        ...
        self._captured = hidden[:n].detach().clone(), positions[:n].detach().clone()
