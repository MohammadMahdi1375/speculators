#!/bin/bash
# ============================================================================
# Online DSpark training on Ascend NPU
# Qwen3-4B + preprocessed Open-PerfectBlend + CANN 9.1.0
#
# NPU split:
#   vLLM hidden-state server : NPUs 0,1
#   DSpark trainer           : NPUs 2,3,4,5,6,7
#
# IMPORTANT:
#   - Dataset is ALREADY preprocessed with Hugging Face save_to_disk.
#   - DO NOT run prepare_data.py again.
#   - Draft vocabulary is explicitly set equal to the full Qwen3-4B
#     verifier vocabulary:
#
#         verifier vocab = 151936
#         draft vocab    = 151936
#
# ============================================================================
...
# Prefer the conda C++ runtime over the older system /usr/lib64 runtime.
...
# Do NOT pass it through prepare_data.py again.
# ---------------------------------------------------------------------------
...
# Do not mix this run with the old draft_vocab_size=32000 checkpoint.
# ---------------------------------------------------------------------------

OUTPUT_DIR="/home/n84449292/m84379596/DFlash/vLLM_NPU_spec_main/output/dspark_qwen3_4b_full_vocab_openperfectblend_rnn"
# ============================================================================
...
#
# CRITICAL:
# Explicitly use full Qwen3-4B vocabulary for BOTH verifier and drafter.
# ============================================================================
...
import speculators
from speculators.models import DSparkDraftModel
...
if "vocab_size" not in cfg:
    raise SystemExit(
        "ERROR: vocab_size not found in verifier config."
    )
...
print("DSpark model     :", DSparkDraftModel)
...
print(
    f"Using FULL vocabulary: "
    f"draft={draft_vocab}, verifier={actual_vocab}"
)
...
if not dataset_info.exists():
    ...
    raise SystemExit(
        ...
        "This does not look like a Hugging Face save_to_disk dataset."
    )
...
if not arrow_files:
    ...
    raise SystemExit(
        "ERROR: no data-*.arrow files found."
    )
...
echo "LR                    : $LR"
echo
echo "MARKOV_RANK           : $MARKOV_RANK"
echo "MARKOV_HEAD_TYPE      : $MARKOV_HEAD_TYPE"
echo
echo "LOSS_FN               : $LOSS_FN"
echo "CONFIDENCE_HEAD_ALPHA : $CONFIDENCE_HEAD_ALPHA"
echo
...
# ============================================================================
# Step 1: Ensure vLLM port is free
# ============================================================================
...
VLLM_LOG="./logs/dspark_qwen3_4b_full_vocab_vllm_server.log"
...
cleanup() {
    ...
    echo
    echo "============================================================"
    echo " Stopping DSpark hidden-state vLLM server"
    echo "============================================================"
    echo
...
echo
echo "============================================================"
echo " Step 3: Using existing preprocessed dataset"
echo "============================================================"
echo


echo "Skipping prepare_data.py."
echo
...
echo "  is explicitly passed to train.py."
echo
...
ASCEND_RT_VISIBLE_DEVICES="$TRAIN_NPUS" torchrun \
    ...
    --logger tensorboard \
    --run-name dspark_qwen3_4b_full_vocab_openperfectblend_npu \
    --log-dir ./logs/dspark_qwen3_4b_full_vocab_openperfectblend_npu \
    --on-missing generate \
    --on-generate delete \
    --request-timeout 180 \
    --max-retries 8 \
...
echo "  ./logs/dspark_qwen3_4b_full_vocab_openperfectblend_npu"
echo
...
if [ -n "${LATEST_CONFIG:-}" ] && [ -f "$LATEST_CONFIG" ]; then
    ...
    echo "============================================================"
    echo " Verifying saved DSpark configuration"
    echo "============================================================"
    echo
...
expected = int(
    os.environ["EXPECTED_DRAFT_VOCAB"]
)
...
if (
    ...
    and
    int(transformer_vocab) != expected
):
...
print()
print(
    f"draft_vocab_size = {expected}"
)
print(
    f"target vocab_size = {expected}"
)
