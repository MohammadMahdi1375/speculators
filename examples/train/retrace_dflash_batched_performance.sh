#!/usr/bin/env bash
# Same checkpoint and training recipe, comparing serial and batched rollouts.
set -eo pipefail
...
RETRACE_BATCH_NPUS="${RETRACE_BATCH_NPUS:-0,1,2,3,4,5,6,7}"
...
export ASCEND_RT_VISIBLE_DEVICES="$RETRACE_BATCH_NPUS"
export RETRACE_BATCH_NPUS
export PYTHONUNBUFFERED=1
export OMP_NUM_THREADS=1
exec "$RETRACE_PYTHON" -m speculators.models.retrace.batched_benchmark "$@"
