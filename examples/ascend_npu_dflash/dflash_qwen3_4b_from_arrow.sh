...
# bash examples/ascend_npu_dflash/dflash_qwen3_4b_from_arrow.sh 2>&1 | tee logs/qwen3_4b_from_arrow.log
#   - verifier model  : Qwen3-8B -> Qwen3-4B
#   - dataset         : existing Arrow dataset open_perfectblend.qwen3-4b-rollout.qwen3.seq3072
...
DATA_DIR="/home/n84449292/m84379596/Huggingface/datasets/open_perfectblend.qwen3-4b-rollout.qwen3.seq3072"
OUTPUT_DIR="./output/dflash_separate_qwen3_4b_from_arrow"
VLLM_PORT=8000
...
VLLM_NPUS="0,1"           # 2 NPUs serve hidden states, DP=2
TRAIN_NPUS="2,3,4,5,6,7"  # 6 NPUs train, draft DP=6
NUM_TRAIN_NPUS=6
VLLM_DP=2
# =======================================================================
...
echo "=== Step 1: using existing Arrow dataset ==="
echo "DATA_DIR=$DATA_DIR"

test -f "$DATA_DIR/state.json"
test -f "$DATA_DIR/dataset_info.json"
test -f "$DATA_DIR/token_freq.pt"
ls "$DATA_DIR"/data-*.arrow >/dev/null
...
ASCEND_RT_VISIBLE_DEVICES="$TRAIN_NPUS" torchrun \
    ...
    --logger tensorboard \
    --run-name dflash_separate_qwen3_4b_from_arrow \
    --log-dir ./logs/separate_qwen3_4b_from_arrow \
    --on-missing generate \
    --on-generate delete \
    --request-timeout 180 \
    --max-retries 8 \
...
echo "Done. Checkpoints: $OUTPUT_DIR/checkpoints/  |  TB logs: ./logs/separate_qwen3_4b_from_arrow"